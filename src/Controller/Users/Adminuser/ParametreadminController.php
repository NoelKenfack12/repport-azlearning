...
public function parametresadmin(GeneralServicetext $service)
{
	...
	if($nbparametre != count($liste_param))
	{
		...
		$oldparam18 = $em->getRepository(Parametreadmin::class)
	                     ->findOneBy(array('type'=>'templatewebsite'));
		if($oldparam18 == null)
		{
			$param18 = new Parametreadmin($service);
			$param18->setType('templatewebsite');
			$param18->setDescription('Permettre la spécification du thème de la plateforme');
			$param18->setValeur('defaultbrand');
			$param18->setUser($this->getUser());
			$em->persist($param18);
		}
